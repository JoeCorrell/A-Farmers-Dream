...
def create_loot_table(color, loot_table_output_directory):
        ...
        "pools": [
            {
                ...
                "entries": [
                    {
                        "type": "item",
                        "name": f"block:kitchen_drawer_wood_{color}",
                        "weight": 1,
                        "functions": [
    ...
    loot_table_json_file_path = os.path.join(
        loot_table_output_directory, f"kitchen_drawer_wood_{color}.loot.json"
    )
    save_json_to_file(loot_table_template, loot_table_json_file_path)

    print(
        f'JSON loot table file "kitchen_drawer_wood_{color}.loot.json" has been created in the directory "{loot_table_output_directory}".'
    )
...
for color in colors:
    ...
    formatted_file_name = f"kitchen_drawer_wood_{color}"
    block_template = {
    ...
    item_json_file_path = os.path.join(
        output_directory, f"kitchen_drawer_wood_{color}.block.json"
    )
    save_json_to_file(block_template, item_json_file_path, overwrite=True)

    print(
        f'JSON file "kitchen_drawer_wood_{color}.block.json" has been created in the directory "{output_directory}".'
    )
    ...
    recipe_template = {
        ...
        "minecraft:recipe_shaped": {
            "description": {
                "identifier": f"block:kitchen_drawer_wood_{color}"
            },
            "priority": 0,
            "tags": ["crafting_table"],
            "pattern": ["LLL", "LCL", "LCL"],
            ...
            "result": f"block:kitchen_drawer_wood_{color}",
        },
    ...
    recipe_json_file_path = os.path.join(
        recipe_output_directory, f"kitchen_drawer_wood_{color}.recipe.json"
    )
    save_json_to_file(recipe_template, recipe_json_file_path, overwrite=True)

    print(
        f'JSON recipe file "kitchen_drawer_wood_{color}.recipe.json" has been created in the directory "{recipe_output_directory}".'
    )
